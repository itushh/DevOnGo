...
export default function GameSessionPage() {
  ...
  const handleStartGame = async () => {
    ...
    if (session.mode !== 'bot' && session.mode !== 'solo' && players.length < 2) {
      alert('You need at least 2 players to start this match.');
      return;
    }
  ...
  const handleLeaveMatch = () => {
    if (window.confirm("Are you sure you want to abandon the match? You may lose your stakes or progress!")) {
       navigate('/game/lobby');
    }
  };
